questions = [
    ...
    {
        "question": "A media analytics company wants to use Amazon Bedrock to generate nightly reports summarizing 50,000 customer-submitted product reviews. Processing must occur between 02:00 and 06:00 UTC daily. The reports are not time-sensitive—results can be delivered by 08:00 UTC. The company currently invokes foundation models (FMs) synchronously at peak hours, which incurs throttling costs and competes with the company's production inference workloads. The company wants to eliminate the competition for FM capacity and reduce per-token costs without provisioning dedicated model units. Which solution will meet these requirements with the LEAST operational overhead?",
        "options": [
            "A) Purchase provisioned throughput for the FM and schedule AWS Lambda functions with Amazon EventBridge Scheduler to invoke the model asynchronously at 02:00 UTC. Use Amazon SQS to buffer review batches and control concurrency.",
            "B) Submit an Amazon Bedrock batch inference job that uses an S3 input manifest file containing all 50,000 reviews. Configure the job to start at 02:00 UTC by using an EventBridge Scheduler rule. Write output to a designated Amazon S3 prefix for downstream report generation.",
            "C) Deploy the FM on an Amazon SageMaker asynchronous inference endpoint with auto-scaling. Use an Amazon SQS queue to buffer reviews and trigger Lambda functions to invoke the endpoint. Scale down the endpoint after processing is complete.",
            "D) Use Amazon Bedrock on-demand inference inside an AWS Glue job. Partition the 50,000 reviews across multiple Glue workers. Configure Glue job bookmarks to restart from the last checkpoint if the job exceeds the nightly window.",
        ],
        "answer": "B",
        "answers": ["B"],
        "multi": False,
        "explanation": "Amazon Bedrock batch inference is purpose-built for non-real-time, high-volume workloads. It accepts an S3 manifest, processes records asynchronously during off-peak windows, and offers discounted per-token pricing compared to on-demand inference—without requiring provisioned throughput or competing with live traffic. EventBridge Scheduler triggers the job at the desired time with zero custom orchestration. Option A requires purchased provisioned throughput (added cost) and custom Lambda orchestration. Option C introduces SageMaker endpoint management overhead. Option D uses on-demand inference inside a Glue job, which still competes for capacity and lacks batch pricing."
    },
    {
        "question": "A life-sciences company wants to adapt a publicly available Amazon Bedrock foundation model (FM) for automated extraction of adverse drug event (ADE) entities from clinical trial notes. The company has a labeled dataset of 80,000 note-entity pairs. The extraction task requires the model to learn domain-specific jargon not present in general pretraining data, follow a strict JSON output schema, and score above 0.92 F1 on an internal test set. Inference will run on batches of notes during nightly processing. Which approach will MOST effectively meet the accuracy and schema-compliance requirements?",
        "options": [
            "A) Enable model access for the base FM in Amazon Bedrock and use few-shot prompt engineering with ten representative note-entity pairs in the system prompt. Invoke the model synchronously for each note during nightly processing.",
            "B) Use Amazon Bedrock to run a fine-tuning job (instruction fine-tuning) on the 80,000 labeled note-entity pairs formatted as instruction-completion records. Deploy the resulting custom model and invoke it via provisioned throughput during nightly batch processing.",
            "C) Use Amazon Bedrock to run a continued pre-training job on a large unlabeled corpus of publicly available biomedical literature to inject domain vocabulary. Invoke the pre-trained model with a detailed system prompt for ADE extraction.",
            "D) Use Amazon Bedrock Agents with a custom action group backed by an AWS Lambda function that calls Amazon Comprehend Medical to detect entities. Pass the agent output through a prompt that reformats results into the required JSON schema.",
        ],
        "answer": "B",
        "answers": ["B"],
        "multi": False,
        "explanation": "Instruction fine-tuning on labeled task-specific data directly optimizes the model for the ADE extraction task—including domain jargon, schema compliance, and F1 performance. Amazon Bedrock supports fine-tuning jobs that consume labeled instruction-completion datasets stored in S3. Option A (few-shot) is unlikely to reach 0.92 F1 on a specialized clinical task. Option C (continued pre-training) injects vocabulary but does not teach the JSON output format or task-specific extraction behavior—a supervised fine-tuning step is still needed. Option D adds orchestration complexity and depends on Comprehend Medical's fixed entity types, which may not cover all ADE-specific jargon."
    },
    {
        "question": "A global insurance company runs a multi-agent Amazon Bedrock system. A supervisor agent orchestrates three specialized collaborator agents: a policy-lookup agent, a claims-history agent, and a fraud-risk-scoring agent. During a peak period, operations teams observe that the supervisor agent occasionally routes identical sub-tasks to the wrong collaborator, and the collaborator agents sometimes return responses that contradict each other when summarizing overlapping claim windows. The company needs to improve routing accuracy and ensure that contradictory sub-agent outputs are reconciled before the supervisor synthesizes a final response. Which combination of changes will address both issues? (Select TWO.)",
        "options": [
            "A) Add detailed, task-specific descriptions to each collaborator agent's instruction block and to the supervisor agent's collaboration configuration so the supervisor can accurately select the correct collaborator for each sub-task.",
            "B) Enable Amazon Bedrock Agents memory so the supervisor retains prior routing decisions across sessions and avoids re-routing the same sub-task to multiple collaborators.",
            "C) Add a reconciliation step in the supervisor agent's orchestration prompt that explicitly instructs it to compare collaborator responses for date-range overlaps and resolve discrepancies before generating the final answer.",
            "D) Configure separate Amazon Bedrock Knowledge Bases for each collaborator agent and enable cross-knowledge-base querying so collaborators can validate each other's facts in real time.",
            "E) Switch the supervisor agent to use an Amazon Bedrock inline agent configuration to reduce inter-agent latency and eliminate routing ambiguity.",
        ],
        "answer": "A, C",
        "answers": ["A", "C"],
        "multi": True,
        "explanation": "Routing errors stem from insufficient agent descriptions—the supervisor cannot disambiguate overlapping capabilities without clear, distinct task scopes in each collaborator's configuration (A). Contradictory outputs stem from the supervisor blindly concatenating sub-responses; an explicit reconciliation instruction in the supervisor's prompt teaches it to detect and resolve overlaps (C). Option B (memory) retains session history but does not fix routing logic or contradiction resolution. Option D adds retrieval infrastructure that does not inherently reconcile conflicting reasoning. Option E (inline agent) reduces deployment overhead but does not resolve routing or reconciliation logic."
    },
    {
        "question": "An e-learning platform stores course content in an internal wiki (Confluence), a SharePoint document library, and a collection of PDF syllabi in Amazon S3. The company wants to build a student-facing Q&A assistant using Amazon Bedrock Knowledge Bases. Answers must always cite the specific page or document they came from. The content is updated daily by instructors. The company has a small ML team and wants to minimize custom ETL code. Which solution will meet these requirements with the LEAST development effort?",
        "options": [
            "A) Write a nightly AWS Glue ETL job that extracts text from Confluence, SharePoint, and S3 PDFs, stores chunks in Amazon OpenSearch Serverless, and triggers a Knowledge Base sync. Invoke the RetrieveAndGenerate API with source attribution enabled.",
            "B) Configure three Amazon Bedrock Knowledge Base data sources—one for the Confluence connector, one for the SharePoint connector, and one for the S3 bucket—within a single Knowledge Base. Schedule incremental syncs daily. Use the RetrieveAndGenerate API with citations enabled to return source metadata to students.",
            "C) Deploy an AWS Lambda function that uses the Confluence REST API and SharePoint Graph API to extract content and write it to an S3 staging bucket. Configure the Knowledge Base to use the S3 bucket as its sole data source. Trigger a Lambda sync after each nightly dump.",
            "D) Use Amazon Kendra as the retrieval engine. Configure Kendra connectors for Confluence, SharePoint, and S3. Build a custom orchestration layer with AWS Lambda that calls Kendra Retrieve and then passes results to an Amazon Bedrock FM for answer generation.",
        ],
        "answer": "B",
        "answers": ["B"],
        "multi": False,
        "explanation": "Amazon Bedrock Knowledge Bases natively supports managed connectors for Confluence, SharePoint, Salesforce, and S3 as first-party data sources, eliminating custom ETL code. Multiple data source connectors can coexist in a single Knowledge Base and be synced on a schedule. The RetrieveAndGenerate API returns source attribution (page URL, document name) out of the box. Option A requires custom Glue ETL development. Option C requires custom Lambda extraction code for two connectors. Option D requires a separate Kendra deployment and a custom orchestration layer, increasing cost and complexity."
    },
    {
        "question": "A government agency is deploying Amazon Bedrock for a citizen-facing chatbot that must operate exclusively within the GovCloud (US-East) region. An AWS Organizations Service Control Policy (SCP) already denies `bedrock:*` for all accounts except the dedicated AI account. The agency uses AWS Control Tower and wants to enforce that: (1) only approved foundation models (a specific list of model IDs) can be invoked, (2) all invocations must originate from within a designated VPC, and (3) model invocation logs must be delivered to a centralized logging account. The security team needs to enforce these constraints without modifying individual IAM role policies in the AI account. Which combination of controls will meet all three requirements? (Select TWO.)",
        "options": [
            "A) Add a `StringNotEquals` condition on `bedrock:ModelId` to an SCP applied to the AI account's OU to restrict invocations to the approved model ID list. Add a `StringNotEquals` condition on `aws:SourceVpc` to deny requests not originating from the designated VPC.",
            "B) Create an IAM permission boundary attached to all roles in the AI account that restricts `bedrock:InvokeModel` to approved model ARNs and includes a `StringEquals` condition on `aws:SourceVpc`.",
            "C) Enable Amazon Bedrock model invocation logging in the AI account and configure the log destination to an Amazon S3 bucket owned by the centralized logging account, protected by a bucket policy that grants cross-account `s3:PutObject` only from the AI account's Bedrock service principal.",
            "D) Deploy an AWS Config rule in the AI account that checks whether Amazon Bedrock invocation logs are enabled and triggers an SSM Automation remediation runbook to re-enable logging if it is disabled.",
            "E) Use Amazon CloudWatch Logs metric filters in the AI account to detect calls to non-approved model IDs and trigger an SNS alarm to the security team.",
        ],
        "answer": "A, C",
        "answers": ["A", "C"],
        "multi": True,
        "explanation": "SCPs are the correct mechanism to enforce policy centrally without modifying IAM roles in member accounts. A single SCP with `StringNotEquals` on `bedrock:ModelId` and `aws:SourceVpc` enforces both model allowlisting and VPC-origin constraints across the OU (A). Centralized invocation logging is configured at the Amazon Bedrock service level with an S3 destination in the logging account—cross-account log delivery is supported by granting the Bedrock service principal permission to write to the logging bucket (C). Option B (permission boundaries) must be attached per-role, requiring modification of each role. Option D (Config remediation) is reactive, not preventive, and introduces a remediation gap. Option E (metric filters + SNS) is alerting only, not enforcement."
    },
    {
        "question": "A financial technology company is building an Amazon Bedrock Agent to automate loan pre-qualification workflows. The agent must: collect applicant income and credit score data from an internal CRM via HTTPS API, query a risk-scoring microservice that requires an API key rotated every 24 hours, validate that the applicant's requested loan amount does not exceed regulatory caps stored in AWS Systems Manager Parameter Store, and call an Amazon Bedrock Knowledge Base for product eligibility rules. The security team mandates that API keys must never be stored in environment variables or Lambda code. Which architecture correctly satisfies all requirements with the LEAST operational overhead?",
        "options": [
            "A) Create an Amazon Bedrock Agent with three action groups: one backed by an AWS Lambda function that retrieves the CRM data via HTTPS, one backed by a Lambda function that fetches the API key from AWS Secrets Manager at invocation time before calling the risk-scoring service, and one backed by a Lambda function that reads the regulatory caps from Parameter Store. Attach the Knowledge Base directly to the agent. Grant the Lambda execution roles least-privilege IAM permissions to Secrets Manager and Parameter Store.",
            "B) Create an Amazon Bedrock Agent with two action groups: one Lambda function that aggregates all external API calls (CRM, risk service, and Parameter Store) and applies retry logic, and one Lambda function for Knowledge Base lookups. Store the risk-scoring API key as an encrypted Lambda environment variable using an AWS KMS customer-managed key.",
            "C) Create an Amazon Bedrock Agent with four action groups. Store API keys in Amazon DynamoDB encrypted with AWS KMS. Have each Lambda function query DynamoDB at invocation time to retrieve the current API key.",
            "D) Deploy the agent orchestration logic in an Amazon ECS Fargate task. Store the risk-scoring API key in an AWS Systems Manager Parameter Store SecureString parameter. Mount the parameter as an ECS environment variable via task definition secrets. Use the Amazon Bedrock InvokeAgent API from the ECS task.",
        ],
        "answer": "A",
        "answers": ["A"],
        "multi": False,
        "explanation": "Option A uses AWS Secrets Manager—the purpose-built service for API key rotation—fetched at Lambda invocation time with no keys in code or environment variables. Separate action group Lambdas follow least privilege and separation of concerns. The Knowledge Base is a native agent attachment, reducing custom retrieval code. Option B violates the security mandate by storing the API key in an environment variable (even KMS-encrypted env vars are visible in the Lambda console). Option C uses DynamoDB as a secrets store—an anti-pattern compared to Secrets Manager's native rotation. Option D uses ECS environment variable injection, which still exposes the key in the task definition."
    },
    {
        "question": "A retail bank's AI platform team manages over 40 Amazon Bedrock Agents deployed across 12 AWS accounts. After a model provider updates a foundation model (FM), several agents begin exhibiting degraded output quality. The team needs a governance process to: (1) test all agents against a regression suite before FM updates propagate to production agents, (2) roll back any agent that fails regression within 15 minutes of detection, (3) maintain an immutable record of which FM version each agent used at any point in time, and (4) enforce that no agent can reference a deprecated model ID. Which combination of services and features will meet all four requirements? (Select TWO.)",
        "options": [
            "A) Use Amazon Bedrock Agents aliasing: create a new agent version for each FM update, run Amazon Bedrock model evaluation jobs against each new version using the regression dataset, and promote only passing versions to the PRODUCTION alias. Store alias-to-version mappings in AWS CloudTrail as immutable evidence.",
            "B) Use AWS Config with a custom rule that queries the Amazon Bedrock ListAgentVersions API and flags any agent version that references a deprecated model ID. Trigger an AWS Systems Manager Automation runbook to revert the agent alias to the last known-good version.",
            "C) Store each agent's FM configuration in an Amazon DynamoDB table with versioned writes and TTL. Use an AWS Lambda function to compare the current agent configuration against the approved FM list nightly and revert non-compliant agents.",
            "D) Use AWS Service Catalog to publish approved agent blueprints. Require all teams to provision agents only through Service Catalog. Use AWS CloudFormation drift detection to identify agents that deviate from their blueprint template.",
            "E) Embed model version pinning in each agent's instruction block. Use Amazon CloudWatch metric alarms on InvocationClientErrors to detect quality degradation and trigger a Lambda rollback function via EventBridge.",
        ],
        "answer": "A, B",
        "answers": ["A", "B"],
        "multi": True,
        "explanation": "Agent versioning + aliasing (A) provides the regression-gated promotion workflow (requirement 1 & 2) and the immutable version history (requirement 3)—each agent version is immutable once created. AWS Config with a custom rule (B) continuously evaluates whether any deployed agent references a deprecated model ID (requirement 4) and drives automated rollback via SSM Automation within the 15-minute SLA. Option C (DynamoDB nightly) is too slow for 15-minute rollback and not immutable. Option D (Service Catalog) governs initial deployment but cannot enforce runtime model ID compliance post-deployment. Option E (instruction-block pinning) is unreliable—model IDs in instructions are advisory, not enforced by the Bedrock control plane."
    },
    {
        "question": "A large telecommunications company is implementing an Amazon Bedrock Knowledge Base to power a technical support chatbot. The knowledge base ingests equipment manuals stored as PDFs in Amazon S3. Each manual can be up to 800 pages. Support engineers report that the chatbot retrieves accurate section headings but frequently loses context about the procedures described within those sections—especially for multi-step configuration procedures that span several pages. Re-reading retrieved chunks, the team finds that individual chunks cut procedure steps in half. The team must preserve procedural context without significantly increasing retrieval latency. Which Knowledge Base configuration change will BEST address this issue?",
        "options": [
            "A) Switch from fixed-size chunking (512 tokens, 10% overlap) to semantic chunking with a breakpoint percentile threshold of 70% and a buffer size of 2 sentences, so chunk boundaries are placed at natural language transitions rather than at arbitrary token counts.",
            "B) Switch to hierarchical chunking with large parent chunks of 2,048 tokens and small child chunks of 512 tokens. The retriever uses child chunk embeddings for precision retrieval but returns the enclosing parent chunk to the FM, preserving full procedural context.",
            "C) Increase the fixed-size chunk size to 4,096 tokens with a 50% overlap. This guarantees that all procedure steps fit within a single chunk while the overlap ensures that cross-chunk procedures are always partially captured.",
            "D) Disable chunking entirely and ingest each PDF page as a single document. Configure the retriever to return the top-5 pages to the FM so that multi-page procedures are always included.",
        ],
        "answer": "B",
        "answers": ["B"],
        "multi": False,
        "explanation": "Hierarchical chunking is specifically designed for scenarios where fine-grained retrieval precision (child chunks) must be combined with broader context (parent chunks). The child chunk embedding pinpoints the relevant procedure section; the returned parent chunk provides the full multi-step procedure. This eliminates mid-procedure cuts without bloating the FM context with entire chapters. Option A (semantic chunking at 70%) improves boundary placement but does not guarantee that a retrieved child chunk contains the full parent context. Option C (large fixed chunks with 50% overlap) inflates token consumption and may still miss procedures that span the overlap boundary. Option D (page-level) produces highly variable chunk sizes and may overwhelm the FM context window with irrelevant page content."
    },
    {
        "question": "A logistics company is building a generative AI (GenAI) supply chain assistant using Amazon Bedrock. The assistant must answer natural language questions that require joining structured shipment data (stored in Amazon Redshift) with unstructured freight contract documents (indexed in an Amazon Bedrock Knowledge Base). Example query: 'Which active contracts have shipments delayed more than 48 hours and what are the penalty clauses?' The company needs sub-5-second end-to-end latency and wants to minimize the amount of custom orchestration code. Which architecture will meet these requirements?",
        "options": [
            "A) Build an Amazon Bedrock Agent with two action groups: one backed by an AWS Lambda function that executes a parameterized Amazon Redshift query for delayed shipments, and one that calls the Knowledge Base Retrieve API to fetch contract penalty clauses. The agent's orchestration logic joins the results and generates the final answer.",
            "B) Create an AWS Step Functions Express Workflow that (1) queries Amazon Redshift Data API for delayed shipments, (2) calls Amazon Bedrock RetrieveAndGenerate to search contract documents, and (3) concatenates both result sets into a final Bedrock InvokeModel call. Expose the workflow via Amazon API Gateway.",
            "C) Index the Redshift query results nightly into the same Amazon Bedrock Knowledge Base as the contracts. Configure the RetrieveAndGenerate API to answer both structured and unstructured queries from the unified index.",
            "D) Use Amazon Athena Federated Query to join an S3-based contract text index with a Redshift connector at query time. Pass the Athena result set as context to an Amazon Bedrock FM invocation inside an AWS Lambda function.",
        ],
        "answer": "A",
        "answers": ["A"],
        "multi": False,
        "explanation": "An Amazon Bedrock Agent with distinct action groups maps naturally to this hybrid retrieval problem: one action group handles structured Redshift queries via Lambda (the Data API is synchronous and suitable for sub-5-second responses), and the other calls the Knowledge Base natively (supported as a first-class Knowledge Base action group, minimizing custom code). The agent's built-in ReAct orchestration joins results and synthesizes the answer. Option B (Step Functions) introduces additional state machine overhead and requires custom join logic. Option C (nightly index refresh) makes shipment data stale by hours, violating the real-time requirement. Option D (Athena Federated Query) adds cross-service join complexity and Athena query startup latency that may exceed 5 seconds for cold queries."
    },
    {
        "question": "An enterprise software company is using Amazon Bedrock Flows to implement a multi-stage document processing pipeline. The flow receives a contract PDF, passes it through a summarization node, then a risk-classification node, and finally a clause-extraction node before writing structured output to Amazon DynamoDB. During load testing with 500 concurrent contracts, the team observes that the risk-classification node occasionally returns a confidence score below the acceptance threshold. The team wants the flow to automatically re-invoke the classification node with a higher inference temperature when confidence is low, up to two retry attempts, before routing low-confidence outputs to a human review queue in Amazon SQS. Which Bedrock Flows feature or design pattern supports this requirement with the LEAST custom code?",
        "options": [
            "A) Add a Condition node after the risk-classification prompt node that evaluates the confidence score field in the node output. Route the low-confidence branch back to the risk-classification node (creating a loop), incrementing an attempt counter stored in the flow's session state. After two retries, route to an Output node that writes to SQS.",
            "B) Wrap the risk-classification stage in an AWS Lambda function that implements retry logic with an adjusted temperature parameter and publishes to SQS after two failures. Connect the Lambda node within the Bedrock Flow using a Lambda function node.",
            "C) Configure the risk-classification prompt node's inference parameters with a high temperature and enable automatic retries at the Amazon Bedrock runtime level. Use a downstream Condition node to route to SQS if the final response confidence is still below threshold.",
            "D) Split the flow into two separate flows: a primary flow for standard processing and a secondary flow for retries. Use Amazon EventBridge to detect low-confidence outputs from the primary flow and trigger the secondary retry flow.",
        ],
        "answer": "A",
        "answers": ["A"],
        "multi": False,
        "explanation": "Amazon Bedrock Flows supports Condition nodes that evaluate expressions on prior node outputs and route execution to different downstream nodes, including looping back to an earlier node. A counter stored in the flow session state (or passed as a flow variable) tracks retry attempts, and after exceeding the limit, the Condition routes to an Output node targeting SQS. This implements the entire retry-and-escalation logic natively within the flow with no custom Lambda code. Option B works but requires custom Lambda code—contradicting the 'LEAST custom code' requirement. Option C uses Bedrock runtime-level retries, which apply to transient throttling/errors, not to application-level confidence thresholds. Option D splits into multiple flows with EventBridge, adding significant orchestration complexity."
    },
    {
        "question": "A healthcare payer company uses Amazon Bedrock to power a clinical coding assistant. Hundreds of medical coders access the assistant simultaneously during business hours. The assistant uses a large FM with a 200,000-token context window and frequently receives the same 15,000-token clinical coding guidelines document as part of every system prompt. The company is seeing unexpectedly high token costs and wants to reduce them without sacrificing response quality or changing the coding guidelines retrieval flow. Which solution will reduce per-request token costs for this workload with the LEAST architectural change?",
        "options": [
            "A) Enable Amazon Bedrock prompt caching for the model and prefix the 15,000-token clinical guidelines as the cacheable prefix of the system prompt. Pay the lower cache read token price for subsequent requests that share the same prefix within the cache TTL.",
            "B) Store the clinical guidelines in an Amazon Bedrock Knowledge Base and replace the full document in the system prompt with a RetrieveAndGenerate call that fetches only the three most relevant sections per query. This reduces average tokens per request.",
            "C) Use Amazon Bedrock Prompt Management to version the system prompt template and set the maximum token length for the guidelines section to 5,000 tokens, truncating the document.",
            "D) Purchase provisioned throughput for the FM. Provisioned throughput pricing is per-model-unit-hour and does not charge per input token, effectively eliminating the repeated guidelines cost.",
        ],
        "answer": "A",
        "answers": ["A"],
        "multi": False,
        "explanation": "Prompt caching allows a cacheable prefix (the stable 15,000-token guidelines) to be processed once and reused across many requests within the cache TTL. Subsequent requests that share this prefix incur a significantly reduced cache read token price instead of the full input token price, directly reducing cost without changing the retrieval architecture. Option B reduces tokens but changes the retrieval flow and may reduce response quality if the wrong sections are retrieved. Option C truncates the document, which can harm coding accuracy. Option D (provisioned throughput) charges per model unit per hour and is cost-effective for sustained, predictable high-throughput workloads, but it does not eliminate per-token billing for on-demand overflow and requires capacity planning."
    },
    {
        "question": "A digital health company is deploying an Amazon Bedrock-powered symptom assessment chatbot for patients. The chatbot must comply with HIPAA and must not store any protected health information (PHI) in logs. The company's security policy requires that all traffic between patient devices and the chatbot backend must traverse AWS PrivateLink, and the FM must not be accessible from the public internet. Additionally, the company's CISO requires automatic detection and blocking of any model response that contains a patient name, date of birth, or medical record number before delivery to the patient. Which architecture satisfies ALL security requirements?",
        "options": [
            "A) Deploy an AWS Lambda function in a private VPC subnet. Create an interface VPC endpoint for `bedrock-runtime`. Disable Amazon Bedrock model invocation logging to prevent PHI in logs. Configure an Amazon Bedrock Guardrail with a sensitive information filter that blocks PII entity types (NAME, DATE_OF_BIRTH, US_HEALTHCARE_ID) applied to model outputs. Expose the Lambda function via a private API Gateway endpoint accessible only through a VPC endpoint.",
            "B) Deploy the chatbot backend on Amazon ECS in a public subnet. Use AWS WAF on the ALB to block requests containing PHI keywords. Configure Amazon Bedrock to invoke the FM over the public internet but encrypt traffic with TLS. Enable CloudWatch Logs with automatic log data protection for PHI.",
            "C) Use Amazon API Gateway with a Cognito authorizer for patient authentication. Enable model invocation logging to Amazon S3 with server-side encryption. Configure a post-processing AWS Lambda function that scans model responses for PHI using Amazon Comprehend Medical before returning responses to patients.",
            "D) Deploy the chatbot on AWS Fargate in a private subnet with a NAT gateway for internet egress to Amazon Bedrock public endpoints. Enable Amazon Macie on the logging S3 bucket to detect and alert on any PHI that inadvertently appears in logs.",
        ],
        "answer": "A",
        "answers": ["A"],
        "multi": False,
        "explanation": "Option A satisfies all requirements: PrivateLink via interface VPC endpoint for `bedrock-runtime` ensures traffic never traverses the public internet; disabling invocation logging prevents PHI from appearing in logs; Amazon Bedrock Guardrails with PII entity filtering on outputs automatically blocks PHI before delivery to patients; private API Gateway ensures patients connect only through VPC. Option B uses public subnets and internet-facing FM access, violating PrivateLink requirements. Option C enables invocation logging (PHI risk) and uses post-processing Lambda instead of the native Guardrails enforcement—adding latency and failure surface. Option D uses a NAT gateway (internet egress), which does not satisfy the 'not accessible from the public internet' requirement for FM traffic."
    },
    {
        "question": "A global asset management firm has deployed Amazon Bedrock across three AWS Regions (us-east-1, eu-west-1, ap-southeast-1) to serve portfolio analysts. The firm is now implementing a chargeback model that allocates Bedrock costs to individual business units (BUs). Each BU runs dozens of applications that all invoke Bedrock FMs. The firm needs: (1) per-BU monthly cost attribution with less than 2% rounding error, (2) automated anomaly alerts when any BU's weekly token spend increases more than 30% versus its 4-week average, (3) a self-service dashboard accessible to BU finance leads without granting them AWS console access. Which combination of services and features will satisfy all three requirements?",
        "options": [
            "A) Tag all Bedrock invocations with a `cost-center` resource tag per BU. Enable AWS Cost and Usage Report (CUR) with resource-level tagging. Use Amazon Athena to query CUR for monthly per-BU breakdowns. Publish weekly token counts as custom CloudWatch metrics with BU dimensions and configure CloudWatch anomaly detection alarms. Embed a CloudWatch dashboard URL in an internal portal for finance leads.",
            "B) Enable Amazon Bedrock model invocation logging with a `requestMetadata` field containing the BU identifier. Ship logs to Amazon S3. Use AWS Glue and Amazon Athena to calculate per-BU token usage and cost using the public Bedrock pricing table. Publish anomaly alerts via Amazon SNS. Build an Amazon QuickSight dashboard with row-level security so each finance lead sees only their BU data.",
            "C) Use AWS Organizations tag policies to enforce the `cost-center` tag on all Bedrock API calls. Configure AWS Cost Explorer grouped by tag to produce monthly BU reports. Set up Cost Explorer anomaly detection per tag value. Share Cost Explorer saved reports with finance leads via IAM read-only console access.",
            "D) Deploy a central API Gateway proxy in front of Bedrock across all Regions. Record BU identifiers in DynamoDB on each invocation. Use a Lambda function to estimate token costs based on response size heuristics. Build a static HTML report generated nightly and posted to an S3 website for finance leads.",
        ],
        "answer": "B",
        "answers": ["B"],
        "multi": False,
        "explanation": "Option B achieves all three requirements: invocation logging with BU metadata in `requestMetadata` captures exact token counts (InputTokenCount, OutputTokenCount) per BU—enabling less-than-2% attribution accuracy when multiplied by published per-token prices. Athena queries on S3 logs produce monthly chargeback reports. Custom SNS-triggered anomaly detection on aggregated weekly token metrics provides BU-level spend alerts. QuickSight with row-level security delivers a self-service dashboard without AWS console access. Option A relies on resource tags, but Bedrock API invocations are not taggable resources in the traditional sense—tags on the IAM role don't appear in CUR at invocation granularity. Option C (Cost Explorer anomaly detection by tag) has coarser granularity and requires console access for finance leads. Option D uses cost heuristics (not exact token counts), failing the 2% accuracy requirement."
    },
    {
        "question": "A SaaS company operates an Amazon Bedrock-powered document Q&A service for enterprise customers. Each customer's documents are stored in a dedicated Amazon S3 prefix. The company uses a single Amazon Bedrock Knowledge Base with Amazon OpenSearch Serverless as the vector store. Customer isolation is enforced by including a `customer_id` metadata attribute on every indexed document. At query time, a metadata filter `{customer_id: <id>}` is applied to the retrieve call. A security audit finds that if the metadata filter is accidentally omitted in application code, cross-customer data leakage is possible. The company needs a defense-in-depth control that prevents cross-customer retrieval even if the application-layer filter is omitted, without migrating to per-customer Knowledge Bases. Which solution adds the required defense-in-depth with the LEAST infrastructure change?",
        "options": [
            "A) Create a separate OpenSearch Serverless collection per customer. Update the Knowledge Base to use dynamic collection routing based on the calling IAM principal's session tags. Grant each customer's IAM role access only to its own collection via resource-based policies.",
            "B) Create an AWS Lambda authorizer for the API Gateway endpoint that validates the customer session token and injects a mandatory metadata filter into every Bedrock RetrieveAndGenerate request before forwarding it to the Knowledge Base.",
            "C) Enable Amazon OpenSearch Serverless field-level security (document-level security) using a security policy that restricts each IAM role to documents where `customer_id` matches the role's session tag value. Map customer application IAM roles to these security policies so that even a filter-less query returns only the calling customer's documents.",
            "D) Add an Amazon Bedrock Guardrail to the Knowledge Base that detects and blocks responses containing data from a different customer_id than the one in the system prompt. Configure the guardrail with a custom denied topic for cross-customer references.",
        ],
        "answer": "C",
        "answers": ["C"],
        "multi": False,
        "explanation": "OpenSearch Serverless document-level security (DLS) enforces row/document visibility at the storage layer. Even if the application omits the metadata filter, the DLS policy silently restricts the query result set to documents the calling IAM role is authorized to see. This is a true defense-in-depth control that operates independently of application code. Option A requires per-customer infrastructure (violates 'least infrastructure change'). Option B (Lambda authorizer) is application-layer enforcement—a bug could still bypass it, and it requires code changes to the authorization flow. Option D (Guardrails) operates on generated text, not on the retrieval layer—cross-customer chunks could still be retrieved and used to generate the response before the guardrail inspects the output."
    },
    {
        "question": "A publishing company is using Amazon Bedrock to build an automated content moderation pipeline for user-submitted articles. The pipeline must: (1) detect and block hate speech and violent content, (2) verify that factual claims in the article are grounded in a curated fact-check knowledge base, (3) redact author PII from the processed version, and (4) flag articles that contain prohibited advertising topics (e.g., gambling, tobacco). The pipeline processes up to 5,000 articles per hour during peak submission windows. Which Amazon Bedrock Guardrails configuration covers all four requirements?",
        "options": [
            "A) Configure a single guardrail with: content filters set to HIGH for HATE and VIOLENCE categories; contextual grounding check enabled with the fact-check knowledge base as the grounding source and a grounding threshold of 0.75; sensitive information filters for PII entity types (NAME, EMAIL, PHONE) with the ANONYMIZE action; denied topics configured for gambling and tobacco with sample phrases and definitions. Apply the guardrail to both input and output evaluation.",
            "B) Create four separate guardrails—one for content filtering, one for grounding checks, one for PII redaction, and one for topic denial—and chain them sequentially using an AWS Step Functions workflow. This provides isolation and easier debugging of which guardrail triggered.",
            "C) Configure a guardrail with content filters for hate and violence. Use a separate Amazon Comprehend custom classifier for topic detection and a separate Amazon Comprehend PII detection job for redaction. Call all three services in parallel from an AWS Lambda function before invoking the FM.",
            "D) Configure content filters and denied topics in a single guardrail. For grounding and PII, use a pre-processing AWS Lambda function that calls Amazon Bedrock RetrieveAndGenerate (for grounding) and Amazon Comprehend (for PII redaction) before the article reaches the guardrail.",
        ],
        "answer": "A",
        "answers": ["A"],
        "multi": False,
        "explanation": "A single Amazon Bedrock Guardrail can simultaneously enforce content filters (hate/violence), contextual grounding checks against a knowledge source, sensitive information PII redaction, and denied topic blocking—all configurable within one guardrail resource. This covers all four requirements with a single API call and minimal custom code. Applying the guardrail to both input and output ensures end-to-end coverage. Option B (four separate guardrails chained in Step Functions) is over-engineered and introduces latency for each hop. Options C and D use external services for capabilities that Guardrails natively provides, adding complexity and per-service costs without benefit."
    },
    {
        "question": "A streaming media company fine-tuned an Amazon Bedrock foundation model (FM) using supervised fine-tuning on 200,000 proprietary content recommendation examples. The fine-tuned model significantly outperforms the base model on internal benchmarks. However, the fine-tuned model is six times larger (in parameter count) than the base model the company originally planned to deploy, resulting in inference latency that exceeds the 300 ms SLA for real-time recommendation API calls. The company wants to create a smaller, faster model that retains most of the fine-tuned model's recommendation quality. Which Amazon Bedrock capability is MOST appropriate for this use case?",
        "options": [
            "A) Use Amazon Bedrock model distillation to generate a synthetic training dataset from the fine-tuned teacher model's outputs on a large unlabeled content corpus. Fine-tune a smaller student FM using this dataset to transfer the teacher's recommendation knowledge into a more efficient model.",
            "B) Use Amazon Bedrock continued pre-training on the smaller base model with the original 200,000 fine-tuning examples to inject recommendation knowledge without the parameter overhead of the larger FM.",
            "C) Configure Amazon Bedrock provisioned throughput for the larger fine-tuned model and set the number of model units (MUs) to the value required to meet the 300 ms SLA at peak request rate.",
            "D) Quantize the fine-tuned model to INT8 precision using Amazon SageMaker Neo and deploy it on a SageMaker real-time endpoint. Use the SageMaker endpoint as a drop-in replacement for the Bedrock inference endpoint.",
        ],
        "answer": "A",
        "answers": ["A"],
        "multi": False,
        "explanation": "Amazon Bedrock model distillation uses a large, capable teacher model to generate high-quality training signals (synthetic outputs) that are used to train a smaller, faster student model. This is the canonical technique for producing a compact model that approximates the teacher's behavior—directly addressing the latency-vs-quality trade-off. Option B (continued pre-training on the smaller model) uses supervised fine-tuning examples but lacks the teacher's output distribution, so quality transfer is less effective than distillation. Option C (provisioned throughput for the larger model) reduces throttling but does not reduce model size or intrinsic inference latency, which is governed by model parameters, not capacity. Option D (SageMaker Neo quantization) is a valid optimization but requires exporting the model to SageMaker—a platform migration—and INT8 quantization alone may not achieve a 300 ms SLA for a very large model."
    },
    {
        "question": "A financial services company is building an Amazon Bedrock Agent for automated trade settlement dispute resolution. The agent must autonomously: (1) query a trade settlement database via a Lambda action group, (2) call a regulatory rule engine microservice to determine applicable settlement rules, (3) draft a resolution letter using an FM, and (4) submit the draft to a human compliance officer for approval before any external action is taken. The company requires that the agent halts after step 3 and resumes only after explicit human approval, maintaining all intermediate state. Which Amazon Bedrock Agents feature enables this pattern natively?",
        "options": [
            "A) Configure the agent with a `PAUSE` return control in the action group Lambda response after the draft letter is generated. The calling application stores the agent's `sessionId` and `turnToken`, presents the draft to the compliance officer, and resumes the agent session with the officer's decision using the same `sessionId`.",
            "B) Use AWS Step Functions to orchestrate the agent invocations. After step 3, the Step Functions workflow enters a `WaitForTaskToken` state. A human task in Amazon Augmented AI (A2I) sends the task token back to Step Functions upon approval, which then calls the agent's next action group.",
            "C) Implement a custom orchestration loop in the application: invoke the agent for steps 1–3, extract the draft from the final response, store it in Amazon DynamoDB, and then start a new agent session with the draft included in the initial prompt after human approval.",
            "D) Configure an Amazon Bedrock Agent alias with two routing configurations: a draft-generation alias (steps 1–3) and a submission alias (step 4). The application calls the draft alias, waits for human approval via Amazon SNS, and then calls the submission alias with the approved draft.",
        ],
        "answer": "A",
        "answers": ["A"],
        "multi": False,
        "explanation": "Amazon Bedrock Agents natively supports the 'return of control' (RoC) pattern: when an action group Lambda returns a `PAUSE` control action, the agent suspends execution and returns the current state to the caller. The application can persist the `sessionId` and resume the agent with the human's approval decision, and the agent continues from where it paused—maintaining all intermediate context. This is the native HITL pattern for Bedrock Agents without external orchestrators. Option B (Step Functions + A2I) works but requires significant additional orchestration infrastructure. Option C breaks the agent's conversational continuity by starting a new session. Option D (alias-based splitting) is not a native agent feature and requires the application to manually carry state between sessions."
    },
    {
        "question": "A global e-commerce company wants to implement Amazon Bedrock cross-region inference (CRI) for its product description generation workload. The workload currently runs in us-east-1 and experiences token-per-minute (TPM) throttling during US business hours. The company has confirmed that the FM it uses is available in us-east-1, us-west-2, and eu-west-1. Data privacy regulations require that European customer data must not be processed outside the EU. North American customer data may be processed in any AWS Region. The company wants to configure CRI to maximize throughput for North American requests while ensuring EU data stays within the EU. Which configuration achieves BOTH requirements?",
        "options": [
            "A) Create a single US-scoped inference profile (prefix `us.`) for North American requests. For EU requests, create a separate EU-scoped inference profile (prefix `eu.`). Route calls to the appropriate profile based on customer geographic origin in the application layer.",
            "B) Create a single global inference profile that spans all three Regions. Use Amazon Route 53 geolocation routing to direct EU users to the eu-west-1 endpoint and North American users to the us-east-1 endpoint. The CRI profile handles spillover across Regions automatically.",
            "C) Enable cross-Region inference and set the primary Region to us-east-1 with us-west-2 as the secondary for North American requests. Configure a separate, eu-west-1-only provisioned throughput deployment for EU requests with no CRI enabled, ensuring EU data does not leave the EU.",
            "D) Purchase provisioned throughput in us-east-1, us-west-2, and eu-west-1. Configure the application to invoke us-east-1 first and fall back to us-west-2 on throttling for North American traffic. Invoke eu-west-1 directly for EU traffic.",
        ],
        "answer": "A",
        "answers": ["A"],
        "multi": False,
        "explanation": "Amazon Bedrock CRI profiles are scoped by geography: US-scoped profiles (`us.*`) distribute load across US Regions only (us-east-1, us-west-2), and EU-scoped profiles (`eu.*`) distribute only across EU Regions (eu-west-1, eu-central-1, etc.). This natively enforces data residency—EU traffic routed to an EU profile will never be processed in a US Region. North American traffic routed to a US profile gets automatic spillover across US Regions, resolving TPM throttling. Option B (global profile) would allow EU data to spill into US Regions, violating data residency. Option C uses provisioned throughput for EU (acceptable but more expensive) combined with CRI for US—it works but is not the simplest CRI-native solution. Option D (manual fallback with provisioned throughput) requires custom retry logic and does not leverage CRI's automatic load distribution."
    },
    {
        "question": "A company is deploying an internal Amazon Bedrock-powered code review assistant accessible to 3,000 engineers. Engineers submit pull requests (PRs) through a GitLab webhook, which triggers an AWS Lambda function that calls Amazon Bedrock to generate review comments. The ML platform team observes that during large release windows, hundreds of PRs are submitted simultaneously, causing Lambda concurrency exhaustion and Bedrock throttling. The team wants to implement a resilient, cost-optimized queuing architecture that ensures every PR is reviewed within 10 minutes of submission, without requiring provisioned concurrency or provisioned throughput. Which architecture achieves these goals?",
        "options": [
            "A) Place an Amazon SQS standard queue between the GitLab webhook Lambda and the Bedrock-calling Lambda. Set the Bedrock-calling Lambda's reserved concurrency to 50. Configure an SQS visibility timeout of 10 minutes and a dead-letter queue (DLQ) for failures. Use exponential backoff with jitter inside the Lambda for Bedrock throttling retries.",
            "B) Use Amazon EventBridge as the event bus. Configure a rule that matches GitLab PR events and invokes a Step Functions Express Workflow for each PR. The workflow calls Bedrock synchronously with a retry policy (max 5 retries, exponential backoff). Set the workflow timeout to 10 minutes.",
            "C) Place an Amazon SQS FIFO queue between the webhook and Bedrock Lambda. Configure one message group per engineer to ensure per-engineer ordering. Set Lambda reserved concurrency to 10 to prevent Bedrock throttling and rely on SQS message retention for the 10-minute SLA.",
            "D) Configure the GitLab webhook to write PR metadata to Amazon Kinesis Data Streams with 20 shards. Deploy a Lambda consumer with an enhanced fan-out subscription. Set Lambda batch size to 5 and retry on Bedrock throttling by re-queuing to a secondary Kinesis stream.",
        ],
        "answer": "A",
        "answers": ["A"],
        "multi": False,
        "explanation": "SQS standard queue with controlled Lambda reserved concurrency decouples PR submission spikes from FM invocations. Reserved concurrency of 50 smooths out the Bedrock request rate without provisioned throughput. The 10-minute visibility timeout enforces the SLA—if a Lambda fails, the message reappears for retry within the window. Exponential backoff with jitter handles Bedrock throttling gracefully. DLQ captures persistent failures for alerting. Option B (EventBridge + Step Functions Express per PR) works but Step Functions Express has a 5-minute maximum duration, which may not accommodate the full 10-minute SLA plus retries. Option C (FIFO queue with per-engineer message groups) is overly complex for this use case and FIFO throughput limits may not match the burst pattern. Option D (Kinesis) introduces shard management complexity and is better suited for streaming data than discrete task queuing."
    },
]
